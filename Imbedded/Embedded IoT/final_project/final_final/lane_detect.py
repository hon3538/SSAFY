...
def thread_camera():
    ...
    threading.Timer(0.05, thread_camera).start()
...
def pipeline(image):
    ...
    if len(left_line_x)!=0 and len(left_line_y)!=0:
        ...
        line_list.append([left_x_start,max_y,left_x_end, min_y])
    if len(right_line_x)!=0 and len(right_line_y)!=0 :
        ...
        line_list.append([right_x_start,max_y,right_x_end, min_y])
...
def getPos(left, right) :
    #width 320 -> 사실상 315
    #center 157 -> 센터이자 최대 gap 값
    
    left_to_center=abs(left-center)
    right_to_center=abs(right-center)
    #ret 이 음수면 왼쪽으로 치우쳐져있고, 양수면 오른쪽으로 치우쳐있음
    ret=(left_to_center-right_to_center)/2
