...
O(V^2)->배열로 최소거리 찾을때
O(ElogV) ->pq로 최소거리 찾을때
무조건 pq로 하는건아님... 만약 정점개수가 적고 간선개수가 100억개이면
배열로 찾는게 이득일 때도 있다. 면접때...활용

모든 가중치가 양수 그래프 G에서 시작점 S부터 다른 모든 점까지의 최단 거리
음수인 싸이클이 있으면 최단거리를 구할 수 없음
음수 간선이 있어도 음수 싸이클만 없으면 괜찮음..

단, 음수 간선이 있을때 다익스트라를 한번만 돌리면 최종 결과가 최단 거리를 
나타내
