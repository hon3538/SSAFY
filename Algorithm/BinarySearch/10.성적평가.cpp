...
int N;
vector<int>ordered[4]; //정렬될거
vector<int>origin[4]; //원본
bool cmp(int n1, int n2) {
    return n1 > n2;
}
int DAT[1001]; // index: 점수, value: 등수
int ps(int target, vector<int> v) { //해당 점수의 등수 찾기
    int left = 0;
...
int main() {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < N; j++) {
            int input;
            cin >> input;
            ordered[i].push_back(input); // i번쨰 대회에서 j번쨰 사람의 점수   
            origin[i].push_back(input);
        }
    }
    int i = 0;
    for (int i = 0; i < N; i++) { //총대회 점수 합산
        ordered[3].push_back(origin[0][i] + origin[1][i] + origin[2][i]);
        origin[3].push_back(origin[0][i] + origin[1][i] + origin[2][i]);
    }
    for (int i = 0; i < 4; i++) {
        sort(ordered[i].begin(), ordered[i].end(),cmp); //내림차순정렬
    }
    for (int i = 0; i < 4; i++) {
